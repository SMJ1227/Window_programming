...
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM IParam) {
	...
	TCHAR outstr[MAX_Y][MAX_X] = { L"ㅇㅇ"};
	static SIZE size;
	DWORD fileSize = MAX_Y * MAX_X * sizeof(TCHAR), read_size;
	...
	case WM_CREATE:
		...
		break;
	case WM_LBUTTONDOWN:
		hFile = CreateFile(L"test.txt", GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
		if (hFile == INVALID_HANDLE_VALUE) {
			MessageBox(hWnd, L"파일을 열 수 없습니다.", L"오류", MB_OK);
			break;
		}

		memset(str, 0, sizeof(str));
		if (ReadFile(hFile, str, fileSize, &read_size, NULL)) {
			// 파일에서 읽은 데이터의 길이
			read_size /= sizeof(TCHAR); // 바이트 단위에서 TCHAR 단위로 변환
		...
		DWORD written;
		WriteFile(hFile, outstr, lstrlen(outstr) * sizeof(TCHAR), &written, NULL);
		CloseHandle(hFile);
		break;
